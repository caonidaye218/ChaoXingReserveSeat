...
from requests.exceptions import RequestException  # 加上异常捕获
...
class reserve:
    ...
    def _get_page_token(self, url):
        try:
            ...
            logging.error(f"Failed to get page token: {e}")
            return ""
    ...
    def resolve_captcha(self):
        try:
            ...
            response = self.requests.get('https://captcha.chaoxing.com/captcha/check/verification/result', params=params, headers=self.headers)
    ...
    def get_slide_captcha_data(self):
        try:
            ...
            logging.error(f"Failed to get captcha images: {e}")
            return "", "", ""
    ...
    def x_distance(self, bg, tp):
        try:
            ...
            c_captcha_headers = self.headers
    ...
    def submit(self, times, roomid, seatid, action):
        for seat in seatid:
            ...
            while ~suc and self.max_attempt > 0:
                ...
                if not token:
                    logging.error("Skip this attempt due to failure of getting token.")
                    self.max_attempt -= 1
