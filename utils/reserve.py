        ...
        for attempt in range(1, self.max_attempt + 1):
            ...
            try:
                ...
                if result.get("success"):
                    return True
                if "人数过多" in msg:
                    logging.warning("遇到“人数过多”，随机等待后重试...")
    ...
    def submit(self, times, roomid, seatid_list, action):
        ...
        with ThreadPoolExecutor(max_workers=min(len(seatid_list), 3)) as executor:
            ...
            for future in as_completed(future_to_seat):
                if future.result():
                    logging.info(f"已抢到座位[{future_to_seat[future]}]，停止其他尝试")
                    return True
